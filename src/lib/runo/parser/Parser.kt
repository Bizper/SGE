...
import lib.runo.checker.Bean
import lib.runo.resource.*
import util.Log
import java.io.File
import java.net.URL
...
class Parser {
    ...
    companion object {
        ...
        val log = Log.getInstance(Parser.javaClass)
    }
    ...
    fun parse(string: String, isDebug: Boolean): Bean {
        ...
        if(log_flag) log.log("parse complete")
        bean = Bean(root)
        return bean
    }
    ...
    fun parseFile(path: String, isDebug: Boolean): Bean {
        val file = File(path)
        if(log_flag) {
            log.log("input file: ${file.canonicalPath}")
            log.log("start parsing...")
        }
        pointer = 0
    ...
    private fun stateLog(sp: Sp) {
        when(sp.state) {
            State.PARSE_EXPECT_VALUE -> if(log_flag) log.log(sp) else println(sp)
            State.PARSE_INVALID_VALUE -> if(log_flag) log.log(sp) else println(sp)
            State.PARSE_ROOT_NOT_SINGULAR -> if(log_flag) log.log(sp) else println(sp)
        }
    }
